...
An option button creates a button which users can use to toggle through a range of items. Pressing the button will send the index of that item to Csound on a channel named by the channel string. The default value is 0. The first item in the `optionButton` will be seen as a 0.
